    ...
    <nav className='flex flex-col mb-3 lg:mb-0 lg:items-center gap-6 lg:flex-row'>
      {shoppingViewHeaderMenuItem.map((item) => (
        <Link
          key={item.id}
          to={item.path}
          className='text-foreground/60 font-medium transition-colors hover:text-foreground'
          onClick={onItemClick}
        >
          {item.label}
        </Link>
...
function HeaderRightContent({ onItemClick }) {
 ...
 function cartupdate() {
    ...
    dispatch(fetchAllcartProduct(user.id)).then(() => {
  ...
  return (
    <div className='flex lg:items-center lg:flex-row flex-col gap-4'>
      ...
      <Sheet open={openCartSheet} onOpenChange={setOpenCartSheet}>
        ...
        <Button onClick={() => { cartupdate(); }} className='relative' variant='outline' size='icon'>
          <ShoppingCart  className='w-6 h-6' />
          <span className='sr-only'>User Cart</span>
        </Button>
        <UserCartWrapper cartItems={cartproductList} onItemClick={onItemClick} />
      </Sheet>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent side='right' className='w-56'>
          <DropdownMenuLabel>
            loged in as {user?.userName}
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => { navigate('/shop/account'); onItemClick && onItemClick(); }}>
            <CircleUserRound className='mr-2 h-4 w-4' />Account
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => { handleLogout(); onItemClick && onItemClick(); }}>
            <LogOut /> Logout
          </DropdownMenuItem>
